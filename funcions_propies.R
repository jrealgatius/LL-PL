...
etiquetar_valors<-function(dt=dades,variables_factors=conductor_variables,fulla="etiquetes",
  ...
  k<-variables_factors%>%select_("camp", "valor",camp_etiqueta)
  pepe<-k %>% base::split(list(.$camp))
...
extreure_OR<- function (formu="AnyPlaqueBasal~CD5L",dades=dt) {
...
extreure_model_logistic<-function(x="OS4_GSK",y="canvi6M.glipesCAT2",taulavariables=conductorvariables,dades=dades,elimina=c("IDP"),a="") {
  ...
  formu=formula.LOGIT(x,y,conductorvariables) 
  
  resposta<-all.vars(formu)[1]
  fit<-stats::glm(formu, family = binomial, data=dades)

  taula_OR<-formu %>% extreure_OR(dades)
  ...
  taula_editada<-taula_editada %>% 
    ...
    etiquetar_taula("Variable",conductorvariables,"descripcio") %>% 
  ...
  dades_prediccio<-
    ...
    tibble::as_tibble() %>% 
    mutate(event=as.numeric(known.truth=="Yes")) %>% 
...
agregar_problemes<-function(dt=PROBLEMES,bd.dindex="20161231",dt.agregadors=CATALEG,finestra.dies=c(-Inf,0),prefix="DG.",camp_agregador="agr") {
  ...
  # dt=PROBLEMES
  # bd.dindex =temp_dindex
  # dt.agregadors=CATALEG
  # finestra.dies=c(-Inf,0)
  # camp_agregador="AGR_SEC"
  # prefix = "EV2."
  
  # dt=PROBLEMES_temp
  # bd.dindex=temp_bdindex
  # dt.agregadors = CATALEG
  # camp_agregador="AGR_SEC"
  # prefix = "EV."
